...
- **2026-03-19 v6**：修复 Live Photo 视频提取不全的问题（直接在整个 HTML 中搜索，与图片提取使用相同策略）
...
**问题**：有 10 个 Live Photo，但只能提取 4-5 个视频

**原因**：使用 `\"h264\"\\s*:\\s*\\[([^\\]]+)\\]` 匹配数组时，`[^\\]]+` 在遇到嵌套的 `]` 时会提前结束

**解决方案**：不限制在特定结构内，直接在整个 HTML 中搜索所有视频 URL

```java
// 直接搜索所有 masterUrl 和 url
...
**关键点**：
- 与图片提取使用相同的策略：直接在整个 HTML 中搜索
- 通过 URL 特征（包含 `sns-video`）来识别视频
- 使用 `LinkedHashSet` 自动去重
